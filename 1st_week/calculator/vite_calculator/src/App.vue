<!-- 
  ...
  ## 데이터 정의
  ### output: 출력값을 위한 데이터
  ### prev: 이전 입력된 숫자 혹은 연산 결과를 저장하기 위한 데이터
  ### cur: 현재 입력중인 숫자를 저장하기 위한 데이터
  ### operator: 입력한 연산자를 저장하기 위한 데이터
  
  ## 조건
  ### output만 v-model로 양방향 데이터 바인딩
  ### output을 제외한 나머지는 모두 @click 이벤트로 연결
  ### 클릭 이벤트 타입에 따른 이벤트 핸들러로 operation() 함수를 정의

  ## 게산기 로직
  ### 클릭한 버튼에 할당된 값을 확인하고 화면 노출
  ### 연산 기호 입력 시 if 문으로 분기 처리 ( includes() 메서드를 사용해 문자열이 ['+', '-', '*', '/', '=']에 포함되어 있으면 true, 아니면 false를 반환)
  ### 문자열이 포함된 경우 operator에 저장하고 prev에 cur의 값 저장, output과 cur null로 초기화
  ### 등호 입력 시 연산을 위해 저장한 숫자를 숫자형으로 변환하고 operator 데이터에 연산 기호가 저장되어 있는지 확인 후사칙연산 기호면 새로운 연산을 위한 로직을 처리하고, 등호면 연산 결과를 노출

  ## 예외 처리
  ### 1. 숫자 입력 전 연산자 입력 방지
  ### 2. 연산자 및 등호 연속 입력 방지
  ### 3. C 버튼 클릭 시 모든 데이터 초기화

  ## 로직 분리 조건
  ### calculate() 함수: 연산의 전체 흐름 제어
  ### clear() 함수: 초기화
  ### userInput() 함수: 사용자가 입력한 숫자를 저장하는 로직
  ### operation() 함수: 연산 담당 로직
